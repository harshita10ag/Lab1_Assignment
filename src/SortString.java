...
public class SortString {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < ((names.length / 2) + 1); i++) {
			names[i] = sc.next(); // assign name according to the length(starting half string)
		}
		for (int i = ((names.length / 2) + 1); i < names.length; i++) { // half string to length
			names[i] = sc.next();
		}
		Arrays.sort(names); // sorting the array
		...
		if (names.length / 2 == 0) {
			for (int i = 0; i < names.length / 2; i++) {
				...
				System.out.println(names[i].toUpperCase()); // print uppercase
			}
		}
	}
}